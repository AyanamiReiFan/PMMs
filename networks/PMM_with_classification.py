...
class OneModel(nn.Module):
    ...
    def get_prototype(self, query_rgb, support_rgb_batch, support_mask_batch):
        query_feature = self.extract_feature_res(query_rgb)
        # feature concate
        feature_size = query_feature.shape[-2:]

        for i in range(support_rgb_batch.shape[0]):
            support_rgb = support_rgb_batch[i:i+1]
            support_mask = support_mask_batch[i:i+1]
    ...
    def forward_return_prob(self, query_rgb, support_rgb, support_mask):
        # extract support_ feature
        support_feature = self.extract_feature_res(support_rgb)
        ...
        return support_feature, query_feature, vec_pos, out, Prob_map

    def forward_without_prob_map(self, query_rgb, support_rgb, support_mask):
        # extract support_ feature
        support_feature = self.extract_feature_res(support_rgb)
    ...
    def forward_with_other_class(self, query_rgb, support_rgb, support_mask, other_class_info):
        # extract support_ feature
        support_feature = self.extract_feature_res(support_rgb)
        ...
        prob_map_list = [prob_map]

        # 其他类的feature
        for other_rgb, other_mask ,weight in other_class_info:
            other_feature = self.extract_feature_res(other_rgb)
            # other PMMs
            other_vec_pos, other_prob_map = self.PMMs(other_feature, other_mask, query_feature)
            # todo 怎么更精细的在这路分支加权重？
            for i in range(self.num_pro):
                vec = other_vec_pos[i]
                exit_feat_in_ = self.f_v_concate(query_feature, vec, feature_size)
                exit_feat_in_ = self.layer55(exit_feat_in_)
                exit_feat_in = exit_feat_in + exit_feat_in_ * weight

            prob_map_list.append(other_prob_map)

        exit_feat_in = self.layer56(exit_feat_in)
        history_mask  = torch.cat(prob_map_list, dim=1)

        # segmentation
        out, _ = self.Segmentation(exit_feat_in, history_mask)
